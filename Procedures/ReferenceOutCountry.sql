CREATE PROCEDURE spReferenceOutCountry
	@UserID INT = NULL,
...
BEGIN
	...
	BEGIN
		...
		--List all
		SELECT ID, agentName, address, phone, mobile, mail, activateInactivate FROM empAgentoutofCountry;
	...
	BEGIN
		...
		IF EXISTS (SELECT 1 FROM empAgentoutofCountry WHERE @Mobile=mobile)
		BEGIN
		...
		--Insert new post
		INSERT INTO empAgentoutofCountry(agentName, address, phone, mobile, mail, activateInactivate, userDateTime)
	...
	BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM empAgentoutofCountry WHERE @ID=id)
		BEGIN
		...
		--Delete if exists
		DELETE FROM empAgentoutofCountry WHERE @ID=id;
